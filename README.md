...
- `--end-datetime DATETIME`: Upper bound of date and time in format yyyy-MM-ddTHH:mm:ss.SSSz of every transaction. If
  not specified it is set equal to start datetime.
...
- `--ratio-delete-contract DELETE_RATIO`: Generate one entry of contracts with DELETE action every DELETE_RATIO
  contracts.
- `--ratio-ko-outcome KO_DELETE_RATIO`: Generate one entry of contracts with KO import outcome every KO_DELETE_RATIO
  contracts with action DELETE.
